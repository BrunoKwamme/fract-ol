...
void	check_c(t_fractol *fractol)
{
	fractol->cx = -0.745429;
	fractol->cy = 0.05;
}
...
t_fractol	*create_instance(t_fractol *fractol, char **argv)
{
	...
	if (ft_strncmp(argv[1], "mandelbrot", 11) == 0 || ft_strncmp(argv[1], "julia", 6) == 0)
		fractol->name = argv[1];
	else
		return (NULL);
	check_c(fractol);
	...
	fractol->img_adr = mlx_get_data_addr(fractol->image, &fractol->bits_per_pixel, &fractol->size_line, &fractol->endian);
	fractol->color = 0x08CF10A ;
	fractol->zoom = 200;
	fractol->x = 0;
	fractol->y = 0;
	fractol->offset_x = -4.25;
